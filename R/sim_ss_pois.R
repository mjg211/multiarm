...
#' @export
sim_ss_pois <- function(des = des_ss_pois(integer = TRUE), lambda, replicates = 1e5,
                        summary = FALSE) {

  ##### Check input variables ##################################################

  check_multiarm_des_ss_pois(des, T)
  ...
  if (summary) {
    summary_sim_ss(des, lambda, replicates, "pois")
    message("")
  }
  ...
  sim              <- matrix(0, nrow_lambda, 4*des$K + 9)
  for (i in 1:nrow_lambda) {
    sim[i, ]       <-
      sim_ss_pois_internal(lambda[i, ], des$alpha, (i - 1)*replicates,
