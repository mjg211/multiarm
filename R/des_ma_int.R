...
#'
#' \code{des_ma_int()} determines \emph{A}-, \emph{D}-, and \emph{E}-optimal
...
#' des     <- des_ma_int()
#' # An A-optimal design
#' des_A   <- des_ma_int(ratio = "A")
#' # Modifying the number of experimental treatments and the total sample size
#' des_K_N <- des_ma_int(K = 3, N = 100)
#' @seealso \code{\link{an_ma}}, \code{\link{build_ma}}, \code{\link{des_ma}},
#' \code{\link{opchar_ma}}, \code{\link{plot.multiarm_des_ma}},
#' \code{\link{sim_ma}}, and their associated S3 \code{\link{print}} and
#' \code{\link{summary}} generics.
#' @export
des_ma_int <- function(K = 2, N = 183, alpha = 0.05, beta = 0.2, delta1 = 0.5,
  ...
  if (all(K == 1, correction != "none")) {
    correction <- "none"
    warning("correction has been reset to \"none\", given the choice K = 1")
  }

  ##### Print summary ##########################################################

  if (summary) {
    summary_des_ma_int(K, N, alpha, beta, delta1, delta0, sigma, ratio,
                       correction)
    message("")
  }
  ...
  if (K > 1) {
    ...
    diag_sqrt_I                     <- matrix(1/sqrt(diag(CovTau)), 1, 1)
  }
