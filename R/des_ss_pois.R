#' Design a single-stage multi-arm clinical trial for a Poisson distributed
#' primary outcome
#'
#' \code{des_ss_pois()} determines single-stage multi-arm clinical trial designs
...
#' instances, \code{des_ss_pois()} computes the required sample size in each
...
#' \code{"D"}, or \code{"E"} to instruct \code{des_ss_pois()} to compute the
...
#' string. A \code{\link{character}} string indicating the chosen event rate
...
#' \code{FALSE}.
#' @return A \code{\link{list}} of class \code{"multiarm_des_ss_pois"}
...
#'                           power      = "disjunctive")
#' @seealso \code{\link{build_ss_pois}}, \code{\link{gui}},
#' \code{\link{opchar_ss_pois}}, \code{\link{plot.multiarm_des_ss_pois}},
#' \code{\link{sim_ss_pois}}.
#' @export
des_ss_pois <- function(K = 2, alpha = 0.025, beta = 0.1, lambda0 = 5,
                        delta1 = 1, delta0 = 0, ratio = rep(1, K),
  ...
  if (ratio_scenario == "HG") {
    sigma        <- rep(sqrt(lambda0), K + 1)
  } else if (ratio_scenario == "HA") {
    sigma        <- c(sqrt(lambda0), rep(sqrt(lambda0 + delta1), K))
  }
  if (is.character(ratio)) {
    ratio_init   <- ratio
    if (summary) {
  ...
  output        <- list(alpha          = alpha,
                        ...
                        summary        = summary)
  class(output) <- c("multiarm_des_ss_pois", class(output))
  output
