...
#'
#' \code{plot.multiarm_des_dtl_pois()} produces power curve plots for a
#' specified multi-stage drop-the-losers multi-arm clinical trial design
#' assuming the primary outcome is Poisson distributed.
#'
#' @param x A \code{\link{list}} of class \code{"multiarm_des_dtl_pois"}, as
...
#' \code{\link{gui}}, \code{\link{opchar_dtl_pois}}, \code{\link{sim_dtl_pois}}.
#' @method plot multiarm_des_dtl_pois
#' @export
plot.multiarm_des_dtl_pois <- function(x = des_dtl_pois(),
                                       delta_min = -x$delta1,
  ...
  #check_multiarm_des_dtl_pois(x, name = "x")
  ...
  seq_K                       <- 1:K
  plots                       <- list()
  lambda                      <- cbind(rep(lambda0, density),
                                       matrix(0, density, K))
  ...
  opchar_equal_og             <- opchar_equal <-
    opchar_dtl_pois(x, lambda)$opchar
  opchar_equal                <-
