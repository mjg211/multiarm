sim_dtl_norm <- function(des = des_dtl_norm(integer = T), tau, replicates = 1e5,
  ...
  #check_multiarm_des_dtl_norm(des, T)
  ...
  if (summary) {
    #summary_sim_dtl(des, tau, replicates, "norm")
    message("")
  }
  ...
  if (length(des$sigma) == 1) {
    sigma          <- rep(des$sigma, des$Kv[1] + 1)
  } else {
    sigma          <- c(des$sigma[1], rep(des$sigma[2], des$Kv[1]))
  }
  nrow_tau         <- nrow(tau)
  total_replicates <- nrow_tau*replicates
  sim              <- matrix(0, nrow_tau, 4*des$Kv[1] + 12)
  for (i in 1:nrow_tau) {
    sim[i, ]       <-
