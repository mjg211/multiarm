#' Build a single-stage multi-arm clinical trial for a Poisson distributed
#' primary outcome
#'
...
#' @seealso \code{\link{des_ss_pois}}, \code{\link{opchar_ss_pois}},
#' \code{\link{plot.multiarm_des_ss_pois}}, \code{\link{sim_ss_pois}}.
#' @export
build_ss_pois <- function(n = rep(141, 3), alpha = 0.025, beta = 0.1,
                          lambda0 = 5, delta1 = 1, delta0 = 0,
  ...
  if (summary) {
    summary_build_ss_pois(K, n, alpha, beta, lambda0, delta1, delta0,
                          correction, power)
    message("")
  }
  ...
  comp    <- components_ss_pois(alpha, beta, correction, delta0, delta1,
                                integer, K, lambda0, power, n[-1]/n[1], NA, n)
