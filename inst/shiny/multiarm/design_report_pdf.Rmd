...
if (params$correction %in% c("benjamini_hochberg", "hochberg", "holm",
  ...
  cat("The following critical threshold should be used with the chosen",
      "multiple comparison correction:", round(params$pi, 3), ".")
}
```
...
```{r, echo = F, results = "asis", fig.width = 5.25, fig.height = 3.75}
if (params$plots) {
  print(params$plot_global)
} else {
  cat("Not chosen for output: select the box next to 'Plot power curves' to",
      "add informative plots to the report.")
}
```

```{r, echo = F, results = "asis", fig.width = 5.25, fig.height = 3.75}
if (params$plots) {
  print(params$plot_LFC)
}
```
