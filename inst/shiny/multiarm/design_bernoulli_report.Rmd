---
title: "Fixed-sample multi-arm clinical trial design for a Bernoulli distributed primary outcome"
...
- The response rate in the control arm will be assumed to be:
$\pi_0 = \boldsymbol{`r pi0`}$.
...
multiple comparison correction: $\boldsymbol{`r gamma_text`}$.

```{r, echo = F, results = "asis"}
library(knitr)
kable(params$data[, c(1:(K + 1), (2*K + 4):(4*K + 3))])
kable(params$data[, 1:(2*K + 3)])
kable(params$data[, -((K + 2):(4*K + 3))])
```
