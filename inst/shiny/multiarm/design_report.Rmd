...
if (plots) {
  plot_text           <- "will"
  punc_text_1         <- ","
  delta_text          <- paste0("using a treatment effect shift of $\\delta =",
                                "\\boldsymbol{", delta, "}$ where required")
  punc_text_2         <- "."
} else {
  plot_text           <- "will not"
  punc_text_1         <- "."
  delta_text          <- punc_text_2 <- ""
}
```
...
- Plots __`r plot_text`__ be produced`r punc_text_1` __`r delta_text`__`r punc_text_2`
