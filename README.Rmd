...
Currently, functions are provided to support trials in which the primary outcome
variable is assumed to be either normally or Bernoulli distributed. In total, 14
functions are available. Their naming conventions are such that several
character strings are joined together separated by underscores. The first
character string indicates the type of calculation the function performs (e.g.,
design determination, operating characteristic calculations), and the remainder
which type of design it is for (normally or Bernoulli distributed outcomes):
